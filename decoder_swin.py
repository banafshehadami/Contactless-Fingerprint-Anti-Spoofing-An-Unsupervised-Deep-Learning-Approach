...
weight. Default: 0.0
    ...
    def __init__(self, dim=192, window_size=(8, 8), num_heads=8,
qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1]
- 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
...
0).contiguous()  # Wh*Ww, Wh*Ww, 2
        relative_coords[:, :, 0] += self.window_size[0] - 1  # shift
to start from 0
...
drop_path=0.,):
        ...
        assert 0 <= self.shift_size < self.window_size, "shift_size
must in 0-window_size"
...
ResidualUnit())
        ...
        self.conv_b = nn.Sequential(
            WinBasedAttention(dim=dim, num_heads=num_heads,
...
self.window_size, C)  # nW*B, window_size*window_size, C

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=attn_mask)  # nW*B,
window_size*window_size, C
...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96*2*2,
norm_layer=None):
        super().__init__()
...
- 1) else None,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                norm_layer=norm_layer,
                downsample=PatchSplit if (i_layer < self.num_layers -
1) else None,
                use_checkpoint=use_checkpoint,
                inverse=True)
            self.syn_layers.append(layer)

        self.end_conv = nn.Sequential(nn.Conv2d(embed_dim, embed_dim *
patch_size ** 2, kernel_size=5, stride=1, padding=2),
                                      nn.PixelShuffle(patch_size),
                                      nn.Conv2d(embed_dim, 3,
kernel_size=3, stride=1, padding=1),
                                     )
        self.super_resolution = nn.ConvTranspose2d(3, 3, 4, stride=2, padding=1)
        self.sig = nn.Sigmoid()
    ...
    def forward(self, x):
        Wh, Ww = x.size(2), x.size(3)
        C = self.embed_dim * 8
        y_hat = x.permute(0, 2, 3, 1).contiguous().view(-1, Wh * Ww, C)
        for i in range(self.num_layers):
            layer = self.syn_layers[i]
            y_hat, Wh, Ww = layer(y_hat, Wh, Ww)

        x_hat = self.end_conv(y_hat.view(-1, Wh, Ww,
self.embed_dim).permute(0, 3, 1, 2).contiguous())
